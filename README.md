...
| `parse-past.mjs` | 지난 공고를 긁어 `past.json` (지자체별 회차 이력) 으로 굽는다 |
...
마감된 공고도 상세에 최종 신청건수가 남아 있어 그대로 긁는다. 지자체 단위로 합산해 `past.json` 으로 굽고,
대시보드 표의 `과거` 열(지자체 중앙값)과 하단 `전년도 경쟁률 이력` 버튼(전체 매트릭스)에서 팝업으로 본다.

```bash
node parse-past.mjs                          # 경기남부 · 신혼 Ⅱ(전세형), 최근 3년
node parse-past.mjs 서울 '청년'              # 지역키워드 · 유형정규식
```

물량이 몰린 회차는 배수가 무너진다(안양 348호 1.0배, 시흥 234호 1.4배). 중앙값만 보지 말고 공급호수를 같이 봐야 한다.

## 매물 목록

공고 첨부의 공급주택목록 xlsx 를 `houses.json` 으로 구워 대시보드 팝업에서 보여준다.
매물 단위 경쟁률은 LH 가 공개하지 않는다 — 신청 자체가 단지(주택군) 단위이고 주택은 선정 후 순번으로 고른다.

```bash
node parse-houses.mjs <fileid>   # 공고 첨부 링크의 fileDownLoad('...') 값
```
